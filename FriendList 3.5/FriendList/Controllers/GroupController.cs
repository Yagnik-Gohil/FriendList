...
namespace FriendList.Controllers
{
    public class GroupController : Controller
    {
        ...
        public IActionResult MyGroupsRead([DataSourceRequest] DataSourceRequest request)
        {
            var user = this.HttpContext.Session.GetString("userId");
            return Json(_userService.GetMyGroups(user).MyGroups.ToDataSourceResult(request));
        }
        public IActionResult GroupInvitesRead([DataSourceRequest] DataSourceRequest request)
        {
            var user = this.HttpContext.Session.GetString("userId");
            return Json(_userService.GetMyGroups(user).GroupInvites.ToDataSourceResult(request));
        }
